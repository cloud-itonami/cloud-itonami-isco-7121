(ns roofcoord.governor-test
  (:require [clojure.test :refer [deftest is testing]]
            [roofcoord.store :as store]
            [roofcoord.advisor :as advisor]
            [roofcoord.governor :as governor]))

(defn- fresh-store []
  (let [st (store/mem-store)]
    (store/register-site! st {:site-id "S-1" :name "Elm Street Re-Roof" :address "42 Elm St"})
    (store/register-worker! st {:worker-id "W-1" :site-id "S-1" :name "Kobo Roofer" :role :crew-lead})
    st))
...
(defn- supply-op [cost]
  {:op :coordinate-supply-order :effect :propose :site-id "S-1"
   :materials "roofing shingles" :cost cost :confidence 0.9 :stake :low
...
(deftest hard-on-unknown-op
  (testing "closed op-allowlist enforced — no op finalizes roofing-work execution or overrides safety authority"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (log-op) :op :finalize-roof-work-execution) st)]
      (is (:hard? v))
...
(deftest hard-and-permanent-on-scope-exclusion-finalize-roof-work
  (testing "a proposal to directly finalize a roofing-work-execution decision is a hard, permanent block"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (log-op)
                                          :confidence 0.99
                                          :rationale "proceed with the roof-work now, skip further review")
                            st)]
      (is (:hard? v))
...
(deftest hard-and-permanent-on-scope-exclusion-override-safety-officer
  (testing "a proposal to override a site-safety officer's fall-protection judgment is a hard, permanent block"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (safety-op)
                                          :confidence 0.99
                                          :rationale "override the site-safety officer and proceed anyway")
                            st)]
      (is (:hard? v))
      (is (not (:escalate? v)))
      (is (some #(= :scope-exclusion-violation (:rule %)) (:violations v))))))
...
(deftest scope-exclusion-not-triggered-by-bare-domain-nouns
  (testing "bare nouns like 'roof'/'height' are ordinary domain vocabulary, not finalization/override actions"
    (let [proposal {:rationale "proposed schedule-crew-operation for roof job site S-1 at height"
                     :description "crew fully trained on fall-protection equipment for this roof"}]
...
(deftest default-mock-advisor-proposals-never-self-trip-scope-exclusion
  (testing "the mock advisor's own default rationale text, across every allowlisted op, never trips the scope-exclusion guard"
    (let [st (fresh-store)
          ...
          requests [{:site-id "S-1" :op :log-work-record :worker-id "W-1" :task "tear-off"}
                    {:site-id "S-1" :op :schedule-crew-operation :worker-id "W-1" :task "install underlayment"}
                    {:site-id "S-1" :op :flag-safety-concern :worker-id "W-1"
                     :concern-type :fall-hazard :severity :high
                     :description "loose scaffold plank near the roof edge at height"}
                    {:site-id "S-1" :op :coordinate-supply-order :materials "roofing shingles and underlayment"
                     :cost 2500}]]
...
(deftest always-escalates-flag-safety-concern-even-at-high-confidence
  (testing "a surfaced fall-hazard/weather-condition/crew-fatigue concern always requires human review"
    (let [st (fresh-store)
