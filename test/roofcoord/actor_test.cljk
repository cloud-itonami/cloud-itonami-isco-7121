(ns roofcoord.actor-test
  (:require [clojure.test :refer [deftest is testing]]
            [roofcoord.actor :as actor]
...
(deftest commits-a-registered-worker-log-work-record
  (let [st (fresh-store)
        ...
        request {:site-id "S-1" :op :log-work-record :stake :low
                 :worker-id "W-1" :task "tear-off north slope"}
...
(deftest commits-a-crew-scheduling-proposal
  (let [st (fresh-store)
        ...
        request {:site-id "S-1" :op :schedule-crew-operation :stake :low
                 :worker-id "W-1" :task "install underlayment"}
...
(deftest holds-a-scope-excluded-proposal-with-no-interrupt-path
  (testing "a proposal to finalize roofing-work execution is a hard, permanent block — never routed through :request-approval"
    (let [st (fresh-store)
          graph (actor/build-graph {:store st})
          request {:site-id "S-1" :op :log-work-record :stake :low
                   :worker-id "W-1" :task "tear-off north slope"
                   :description "proceed with the roof-work now, skip further review"}
...
(deftest interrupts-then-approves-an-above-threshold-supply-order-on-human-approval
  (let [st (fresh-store)
        ...
        request {:site-id "S-1" :op :coordinate-supply-order :stake :low
                 :materials "roofing shingles" :cost 25000}
