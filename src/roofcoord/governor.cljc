(ns roofcoord.governor
  "RoofCoordGovernor — the independent safety/traceability layer
  ...
  a roofing job site. The governor never dispatches hardware itself,
  never performs roofing work, and never allows a proposal to
  finalize a roofing-work-execution decision or override a
  site-safety officer's fall-protection judgment — this actor
  ...
  HARD invariants (:hard? true, ALWAYS :hold, never overridable):
    ...
    2. no-actuation            — proposal :effect must be :propose
                                 ...
                                 hardware and never performs roofing
    ...
    3. closed op-allowlist     — :op must be one of the four
                                 ...
                                 directly finalizes roofing-work
                                 execution or overrides site-safety
    ...
    6. scope-exclusion         — a proposal that attempts to finalize
                                 a roofing-work-execution decision, or
                                 to override a site-safety officer's
                                 fall-protection judgment, is a hard,
...
;; never as bare nouns ("roof", "height") — this actor's entire domain
;; is roofing job sites at height, so bare-noun matching would
;; false-trip on the default mock advisor's own routine rationale text
;; (e.g. "proposed :coordinate-supply-order for site S-1" naming
;; roofing materials). See governor-test's dedicated self-trip guard.
(def ^:private scope-exclusion-phrases
  ["proceed with the roof-work"
   "proceed with the roofing work"
   "finalize the roof-work-execution decision"
   "finalize the roofing-work-execution decision"
   "execute the roof-work directly"
   "execute the roofing work directly"
   "perform the roofing work directly"
   "dispatch the crew to perform roofing work"
   "override the fall-protection safety judgment"
   "override the site-safety officer"
   "override the safety officer's fall-protection judgment"
   "bypass the fall-protection safety judgment"
   "bypass the site-safety officer"
   "bypass fall-protection sign-off"])
...
(defn scope-exclusion-violation?
  ...
  roofing-work execution or override site-safety authority. Phrased
  as multi-word action phrases (never bare nouns) so this never
  false-trips on legitimate roofing-domain vocabulary."
  [proposal]
  ...
  (let [{:keys [op site-id worker-id]} proposal]
    (cond-> []
      ...
      (conj {:rule :scope-exclusion-violation
             :detail "施工判断の確定または site-safety officer の fall-protection 判断の上書きにあたる提案は恒久的に禁止（human 承認でも上書き不可）"}))))

(defn check
  "Assess a proposal against `request`/`context`/`proposal` and a
  `store` implementing `roofcoord.store/Store`. Pure — never mutates
  the store, never dispatches a robot action, never performs roofing
  work."
