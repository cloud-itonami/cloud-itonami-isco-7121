(ns roofcoord.actor
  "RoofCoordActor — the ISCO-08 7121 roofing crew scheduling/logistics
  ...
  roofing work itself and never commits a proposal the
  RoofCoordGovernor refuses. Modeled on cloud-itonami-isco-3313's
  ...
  The unconditional invariant: the Roof Coordination Advisor can never
  directly commit a coordination record the RoofCoordGovernor refuses
  — every commit-record! call is gated behind `:decide`, and the
  governor's scope-exclusion rule makes any proposal to finalize
  roofing-work execution or override a site-safety officer's
  fall-protection judgment an unconditional, permanent :hold."
...
(defn build-graph
  "Build a compiled RoofCoordActor graph. `store` implements
  `roofcoord.store/Store`. `advisor` implements
  `roofcoord.advisor/Advisor` (defaults to `mock-advisor`).
...
(defn approve!
  ...
  can never be used to approve a roofing-work-execution finalization
  or a fall-protection-judgment override."
  [graph thread-id]
